...
class Rudra_wash_types_apis extends CI_Controller
{
    ...
    /***********************Page Route
     //rudra_wash_types API Routes
	$t_name = 'auto_scripts/Rudra_wash_types_apis/';
	$route[$api_ver.'wash_types/(:any)'] = $t_name.'rudra_rudra_wash_types/$1';
    ...
    public function rudra_rudra_wash_types($param1)
    {
        $call_type = $param1;
        $res = array();
        if ($call_type == 'put') {
        ...
        } elseif ($call_type == 'dropdown-list') {
            $res = $this->washDropdown($_POST);
    ...
    public function rudra_save_data()
    {
        if (!empty($_POST)) {
            ...
            $this->form_validation->set_rules('wash_name', 'wash_name', 'required');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $updateArray =
                    array(
                        'wash_name' => $this->input->post('wash_name', true),
    ...
    public function rudra_paged_data()
    {
        if (!empty($_POST)) {
            $this->form_validation->set_rules('api_key', 'API KEY', 'required');
            $this->form_validation->set_rules('car_size', 'car size', 'required');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $query = "SELECT * FROM $this->table WHERE `car_size` = '" . $car_size . "' AND `status` = '1' AND `is_deleted` = '0' ORDER BY `id` DESC LIMIT $start_index , $per_page";
                $result = $this->db->query($query)->result();
                if (!empty($result)) {
                    $list = array();
                    foreach ($result as $res) {
                        if($lang==1){
                            $wash_name = $res->wash_name;
                            $description = $res->description;
                        }else{
                            $wash_name = $res->wash_name_dn;
                            $description = $res->description_dn;
                        }
                        $list[] = [
                            "wash_id" => $res->id,
                            "car_size" => $res->car_size,
                            "car_size_definition" => CarSizeType($id = $res->car_size),
                            "wash_icon" => base_url('app_assets/images/wash-icons/' . $res->icon),
                            "wash_name" => $wash_name,
                            // "wash_name_dn" => $res->wash_name_dn,
                            "wash_amount" => $res->amount,
                            "description" => $description,
                            // "description_dn" => $res->description_dn,
                        ];
                    }
                    $this->chk = 1;
                    $this->msg = getLangMessage($lang, $str = "wash_types_listed_successfully");
    ...
    public function washDropdown()
    {
        if (!empty($_POST)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $where = (isset($input['id']) && $input['id'] != "" && is_numeric($input['id'])) ? " AND id = '" . $input['id'] . "' " : "";

                $query = "SELECT * FROM $this->table WHERE `status` = '1' AND `is_deleted` = '0' " . $where . " ORDER BY `id` DESC; ";
                ...
                if ($result->num_rows() > 0) {
                    foreach ($result->result() as $key => $res) {
                        ...
                        $list[] = [
                            "wash_id" => $res->id,
                            "wash_name" => $wash_name . ' (' .getLangMessage($lang, $str = "car_size"). ': ' . getLangMessage($lang, $str = (strtolower(CarSizeType($id = $res->car_size)))) . ')',
                        ];
                    }
                    $this->chk = 1;
                    $this->msg = getLangMessage($lang, $str = "wash_types_listed_successfully");
