...
class Rudra_slots_apis extends CI_Controller
{
    ...
    /***********************Page Route
     ...
     //rudra_slots API Routes
	$t_name = 'auto_scripts/Rudra_slots_apis/';    
	$route[$api_ver.'slots/(:any)'] = $t_name.'rudra_rudra_slots/$1';
    ...
    public function rudra_rudra_slots($param1)
    {
        $call_type = $param1;
        $res = array();
        if ($call_type == 'put') {
    ...
    public function rudra_save_data()
    {
        if (!empty($_POST)) {
            ...
            $this->form_validation->set_rules('slot_date', 'slot_date', 'required');
            $this->form_validation->set_rules('start_time', 'start_time', 'required');
            $this->form_validation->set_rules('end_time', 'end_time', 'required');
            $this->form_validation->set_rules('no_of_service', 'no_of_service', 'required');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $updateArray =
                    array(
                        ...
                        'no_of_service' => $this->input->post('no_of_service', true),
