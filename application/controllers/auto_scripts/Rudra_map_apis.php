...
class Rudra_map_apis extends CI_Controller
{                   
    ...
    /***********************Page Route
     ...
     //rudra_map API Routes
	$t_name = 'auto_scripts/Rudra_map_apis/';    
	$route[$api_ver.'map/(:any)'] = $t_name.'rudra_rudra_map/$1';
    ...
    **********************************/
    ...
    public function rudra_rudra_map($param1)
    {
        $call_type = $param1;
        $res = array();
        if($call_type == 'put')
       ...
       if(!empty($_POST))
        {
            ...
            { 
                $pk_id = 1;
                $res = $this->db->get_where("$this->table",array('id' => $pk_id, 'status'=> '1', 'is_deleted' =>'0'))->row();
                if(!empty($res))
                {
                    //Format Data if required
                    /*********
