...
class Rudra_booking_apis extends CI_Controller
{
    ...
    public function rudra_save_data()
    {
        if (!empty($_POST)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $extr = '';
                $res = $this->bookingInfo($user_id = $insert_array['user_id'], $booking_id = $new_id, $page = "1",$filter_where = "", $washer_id = "",$lang,$extr);
                ...
                $timeslots = $this->db->get_where('rudra_times',array('day'=>$input['date'], 'time'=>$input['time']))->row(); 
                if($timeslots){
                    $slot = $timeslots->slots_taken + 1;
                    $updateArray = array(
                        'slots_taken' => $slot
                    );
                
                    $this->db->where('id',$timeslots->id);
                    $this->db->update('rudra_times',$updateArray);
    ...
    public function rudra_paged_data($call_type)
    {
        if (!empty($_POST)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $extr = '';
                $list = $this->bookingInfo($user_id, $booking_id = $wash_id, $page, $filter_where, $washer_id = "",$lang,$extr);
    ...
    public function findJobs()
    {
        if (!empty($_POST)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if($zipcode != "")
                {
                     //$filter_where .= " AND `b`.`zipcode` ='" . $zipcode . "' ";
                     $get_zip_det = $this->db->get_where("rudra_zipcodes",array("id" => $zipcode))->row();
                     if(!empty($get_zip_det))
                     {
                        $extr = "(6371 * acos( 
                            cos( radians(b.latitude) ) 
                          * cos( radians($get_zip_det->t1 ) ) 
                          * cos( radians($get_zip_det->l1 ) - radians(b.longitude) ) 
                          + sin( radians(b.latitude) ) 
                          * sin( radians($get_zip_det->t1 ) )
                            ) ) as distance, ";
                     }
                     //print_r($get_zip_det);exit;
                ...
                $list = $this->bookingInfo($user_id = "", $booking_id = "", $page, $filter_where, $washer_id = "",$lang,$extr);
    ...
    public function viewBookingDetails()
    {
        if (!empty($_POST)) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $extr = '';
                $list = $this->bookingInfo($user_id = "", $booking_id = "", $page = "1", $filter_where, $washer_id,$lang,$extr);
                $this->chk = 1;
                $this->msg = getLangMessage($lang, $str = "booked_wash_listed_successfully");
                $this->return_data = $list;
            }
    ...
    public function bookingInfo($user_id = "", $booking_id = "", $page = "1", $filter_where = "", $washer_id = "", $lang ="1",$extr)
    {
        $limit = "25";
        $start = "0";
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                foreach ($week_days as $key => $date) {
                    $timings['date'] = $date;
                    $day = date('Y-m-d', strtotime($timings['date']));
                    ...
                    $timeslots = $this->db->order_by('time', 'ASC')->get_where('rudra_times',array('day'=>$day))->result();
                    ...
                    if(count($timeslots) < 1 ){
                        ...
                        for($j=1;$j<=6;$j++){
                            ...
                            $this->db->insert('rudra_times',$updateArray);
                    ...
                    foreach($timeslots as $slot){
                        ...
                        $flag = 0;
                        if($slot->total_slots > $slot->slots_taken){
                            $flag = 1;
                        }

                        $start = date('H:i', strtotime($slot->time));
                        $end = $nextstart = date('H:i', strtotime("+60 minutes",strtotime($slot->time))  );
                        // $checkend = date('h:i', strtotime("+30 minutes", strtotime($slot->time)));

                        $checktime = $this->db->get_where('rudra_times',array('day'=>$day, 'time >' =>  $slot->time , 'time <'=> $nextstart))->row(); 
                        // print_r($checktime);
                        if($checktime){
                            $end = $nextstart = date('h:i', strtotime($checktime->time));
                        }


                        if($slot->status=='1' && $slot->is_deleted=='0'){
                            $slots[] = array(
                                "start" => $start,
