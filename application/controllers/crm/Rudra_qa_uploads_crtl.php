...
class Rudra_qa_uploads_crtl extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('rudra_qa_uploads_rudra_model', 'rudram');
    ...
    /***********
	//Rudra_qa_uploads_crtl ROUTES
        $crud_master = $crm . "Rudra_qa_uploads_crtl/";
        $route['rudra_qa_uploads'] = $crud_master . 'index';
        $route['rudra_qa_uploads/index'] = $crud_master . 'index';
        $route['rudra_qa_uploads/list'] = $crud_master . 'list';
        $route['rudra_qa_uploads/post_actions/(:any)'] = $crud_master.'post_actions/$1';
     **************/

    public function index()
    {
        // main index codes goes here
        $data['pageTitle'] = ' Qa Uploads';
        $data['page_template'] = '_rudra_qa_uploads';
        $data['page_header'] = ' Qa Uploads';
        $data['load_type'] = 'all';
    ...
    public function list()
    {
        ...
        $orderArray = array('id', 'fk_booking_id', 'file_name', 'file_path', 'status', 'is_deleted',);
        ...
        if (!empty($rows)) {
            $res_json = array();
            foreach ($rows as $row) {
                $actions_base_url = 'rudra_qa_uploads/post_actions';
                $actions_query_string = "?id= $row->id.'&target_table='.$row->id";
                $form_data_url = 'rudra_qa_uploads/post_actions';
                $action_url = 'rudra_qa_uploads/post_actions';
                ...
                $row->fk_booking_id = (!empty($fk_booking_id) ? $fk_booking_id->id : '--');
    ...
    public function post_actions($param1)
    {
        ...
        if ($action == "get_data") {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_qa_uploads_form", $data, TRUE);
        ...
        if ($action == "export_data") {
            ...
            $filename = strtolower('rudra_qa_uploads') . '_' . date('d-m-Y') . ".csv";
