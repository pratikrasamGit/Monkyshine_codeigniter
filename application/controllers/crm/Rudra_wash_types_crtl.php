...
class Rudra_wash_types_crtl extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->bdp = $this->db->dbprefix;
        $this->full_table = 'rudra_wash_types';
        $this->return_data = array('status' => false, 'msg' => 'Error', 'login' => false, 'data' => array());
        //$this->set_data();
        $this->base_id = 0;
        $this->load->model('crudmaster_model', 'crd');
        $this->load->model('rudra_wash_types_rudra_model', 'rudram');
    ...
    /***********
	//Rudra_wash_types_crtl ROUTES
        $crud_master = $crm . "Rudra_wash_types_crtl/";
        $route['rudra_wash_types'] = $crud_master . 'index';
        $route['rudra_wash_types/index'] = $crud_master . 'index';
        $route['rudra_wash_types/list'] = $crud_master . 'list';
        $route['rudra_wash_types/post_actions/(:any)'] = $crud_master.'post_actions/$1';
     **************/

    public function index()
    {
        // main index codes goes here
        $data['pageTitle'] = ' Wash Types';
        $data['page_template'] = '_rudra_wash_types';
        $data['page_header'] = ' Wash Types';
        $data['load_type'] = 'all';
    ...
    public function list()
    {
        ...
        $orderArray = array('id', 'car_size', 'icon', 'wash_name', 'amount', 'description', 'status', 'is_deleted',);
        ...
        if (!empty($rows)) {
            $res_json = array();
            foreach ($rows as $row) {
                $actions_base_url = 'rudra_wash_types/post_actions';
                $actions_query_string = "?id= $row->id.'&target_table='.$row->id";
                $form_data_url = 'rudra_wash_types/post_actions';
                $action_url = 'rudra_wash_types/post_actions';
                ...
                //JOINS LOGIC
                $row->icon = (isset($row->icon) && $row->icon != "") ? "<img src='" . base_url('app_assets/images/wash-icons/' . $row->icon) . "' style='height=50px; width:50px;'>" : "";
    ...
    public function post_actions($param1)
    {
        ...
        if ($action == "get_data") {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_wash_types_form", $data, TRUE);
        ...
        if ($action == "update_data") {
            if (!empty($_POST)) {
                ...
                $updateArray =
                    array(
                        ...
                        'wash_name_dn' => $this->input->post('wash_name_dn', true),
                        'amount' => $this->input->post('amount', true),
                        'description' => $this->input->post('description', true),
                        'description_dn' => $this->input->post('description_dn', true),
            ...
            if (isset($_FILES['icon']) && $_FILES['icon']['name'] != '') {
                $bannerpath = 'app_assets/images/wash-icons/';
                $thumbpath = 'app_assets/images/wash-icons/';
        ...
        if ($action == "export_data") {
            ...
            $filename = strtolower('rudra_wash_types') . '_' . date('d-m-Y') . ".csv";
