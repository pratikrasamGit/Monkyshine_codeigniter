...
class Rudra_crud_master_tables_crtl extends CI_Controller
{                   
    public function __construct()
    {
        parent::__construct();                
        $this->bdp = $this->db->dbprefix;
        $this->full_table = 'rudra_crud_master_tables';
        $this->return_data = array('status' => false, 'msg' => 'Error', 'login' => false, 'data' => array());
        //$this->set_data();
        $this->base_id = 0;
        $this->load->model('crudmaster_model','crd');
        $this->load->model('rudra_crud_master_tables_rudra_model','rudram');
    ...
    /***********
	//Rudra_crud_master_tables_crtl ROUTES
        $crud_master = $crm . "Rudra_crud_master_tables_crtl/";
        $route['rudra_crud_master_tables'] = $crud_master . 'index';
        $route['rudra_crud_master_tables/index'] = $crud_master . 'index';
        $route['rudra_crud_master_tables/list'] = $crud_master . 'list';
        $route['rudra_crud_master_tables/post_actions/(:any)'] = $crud_master.'post_actions/$1';
	**************/

    public function index()
    {
        // main index codes goes here
        $data['pageTitle'] = ' Crud Master Tables';                        
        $data['page_template'] = '_rudra_crud_master_tables';
        $data['page_header'] = ' Crud Master Tables';
        $data['load_type'] = 'all';                
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'tbl_name', 'col_strc', 'list_template', 'status', );
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'rudra_crud_master_tables/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'rudra_crud_master_tables/post_actions';
				$action_url = 'rudra_crud_master_tables/post_actions';
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_crud_master_tables_form", $data, TRUE);
        ...
        if($action == "update_data")
        {
            if(!empty($_POST))
            {
				...
				array(
				 ...
				 'tbl_name' => $this->input->post('tbl_name',true),
				 'col_strc' => $this->input->post('col_strc',true),
				 'list_template' => $this->input->post('list_template',true),
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_crud_master_tables').'_'.date('d-m-Y').".csv";
