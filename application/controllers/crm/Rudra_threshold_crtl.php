...
class Rudra_threshold_crtl extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->bdp = $this->db->dbprefix;
        $this->full_table = 'rudra_threshold';
        $this->return_data = array('status' => false, 'msg' => 'Error', 'login' => false, 'data' => array());
        //$this->set_data();
        $this->base_id = 0;
        $this->load->model('crudmaster_model', 'crd');
        $this->load->model('rudra_threshold_rudra_model', 'rudram');
	...
	//Rudra_threshold_crtl ROUTES
        $crud_master = $crm . "Rudra_threshold_crtl/";
        $route['rudra_threshold'] = $crud_master . 'index';
        $route['rudra_threshold/index'] = $crud_master . 'index';
        $route['rudra_threshold/list'] = $crud_master . 'list';
        $route['rudra_threshold/post_actions/(:any)'] = $crud_master.'post_actions/$1';
     **************/

    public function index()
    {
        // main index codes goes here
        $data['pageTitle'] = ' Threshold';
        $data['page_template'] = '_rudra_threshold';
        $data['page_header'] = ' Threshold';
        $data['load_type'] = 'all';
    ...
    public function list()
    {
        ...
        $orderArray = array('id', 'label', 'value', 'status', 'is_deleted',);
        ...
        if (!empty($rows)) {
            $res_json = array();
            foreach ($rows as $row) {
                $actions_base_url = 'rudra_threshold/post_actions';
                $actions_query_string = "?id= $row->id.'&target_table='.$row->id";
                $form_data_url = 'rudra_threshold/post_actions';
                $action_url = 'rudra_threshold/post_actions';
    ...
    public function post_actions($param1)
    {
        ...
        if ($action == "get_data") {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_threshold_form", $data, TRUE);
        ...
        if ($action == "export_data") {
            ...
            $filename = strtolower('rudra_threshold') . '_' . date('d-m-Y') . ".csv";
