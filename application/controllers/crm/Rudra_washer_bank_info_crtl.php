...
class Rudra_washer_bank_info_crtl extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->bdp = $this->db->dbprefix;
        $this->full_table = 'rudra_washer_bank_info';
        $this->return_data = array('status' => false, 'msg' => 'Error', 'login' => false, 'data' => array());
        //$this->set_data();
        $this->base_id = 0;
        $this->load->model('crudmaster_model', 'crd');
        $this->load->model('rudra_washer_bank_info_rudra_model', 'rudram');
    ...
    /***********
	//Rudra_washer_bank_info_crtl ROUTES
        $crud_master = $crm . "Rudra_washer_bank_info_crtl/";
        $route['rudra_washer_bank_info'] = $crud_master . 'index';
        $route['rudra_washer_bank_info/index'] = $crud_master . 'index';
        $route['rudra_washer_bank_info/list'] = $crud_master . 'list';
        $route['rudra_washer_bank_info/post_actions/(:any)'] = $crud_master.'post_actions/$1';
     **************/

    public function index()
    {
        // main index codes goes here
        $data['pageTitle'] = ' Washer Bank Info';
        $data['page_template'] = '_rudra_washer_bank_info';
        $data['page_header'] = ' Washer Bank Info';
        $data['load_type'] = 'all';
    ...
    public function list()
    {
        ...
        $orderArray = array('id', 'u_name', 'registration_number', 'account_number', 'status', 'is_deleted',);
        ...
        if (!empty($rows)) {
            $res_json = array();
            foreach ($rows as $row) {
                $actions_base_url = 'rudra_washer_bank_info/post_actions';
                $actions_query_string = "?id= $row->id.'&target_table='.$row->id";
                $form_data_url = 'rudra_washer_bank_info/post_actions';
                $action_url = 'rudra_washer_bank_info/post_actions';
                ...
                $row->fk_user_id = (!empty($fk_user_id) ? $fk_user_id->id : '--');
    ...
    public function post_actions($param1)
    {
        ...
        if ($action == "get_data") {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_washer_bank_info_form", $data, TRUE);
        ...
        if ($action == "export_data") {
            ...
            $filename = strtolower('rudra_washer_bank_info') . '_' . date('d-m-Y') . ".csv";
