...
class Rudra_chat_uploads_crtl extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->bdp = $this->db->dbprefix;
        $this->full_table = 'rudra_chat_uploads';
        $this->return_data = array('status' => false, 'msg' => 'Error', 'login' => false, 'data' => array());
        //$this->set_data();
        $this->base_id = 0;
        $this->load->model('crudmaster_model', 'crd');
        $this->load->model('rudra_chat_uploads_rudra_model', 'rudram');
    ...
    /***********
	//Rudra_chat_uploads_crtl ROUTES
        $crud_master = $crm . "Rudra_chat_uploads_crtl/";
        $route['rudra_chat_uploads'] = $crud_master . 'index';
        $route['rudra_chat_uploads/index'] = $crud_master . 'index';
        $route['rudra_chat_uploads/list'] = $crud_master . 'list';
        $route['rudra_chat_uploads/post_actions/(:any)'] = $crud_master.'post_actions/$1';
     **************/

    public function index()
    {
        // main index codes goes here
        $data['pageTitle'] = ' Chat Uploads';
        $data['page_template'] = '_rudra_chat_uploads';
        $data['page_header'] = ' Chat Uploads';
        $data['load_type'] = 'all';
    ...
    public function list()
    {
        ...
        $orderArray = array('id', 'fk_chat_id', 'file_name', 'status', 'is_deleted',);
        ...
        if (!empty($rows)) {
            $res_json = array();
            foreach ($rows as $row) {
                $actions_base_url = 'rudra_chat_uploads/post_actions';
                $actions_query_string = "?id= $row->id.'&target_table='.$row->id";
                $form_data_url = 'rudra_chat_uploads/post_actions';
                $action_url = 'rudra_chat_uploads/post_actions';
                ...
                $fk_chat_id = $this->db->get_where('rudra_chat', array('id' => $row->fk_chat_id))->row();
                $row->fk_chat_id = (!empty($fk_chat_id) ? $fk_chat_id->id : '--');
    ...
    public function post_actions($param1)
    {
        ...
        if ($action == "get_data") {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_chat_uploads_form", $data, TRUE);
        ...
        if ($action == "export_data") {
            ...
            $filename = strtolower('rudra_chat_uploads') . '_' . date('d-m-Y') . ".csv";
