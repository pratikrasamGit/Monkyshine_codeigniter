...
{                   
	...
	//Rudra_map_crtl ROUTES
        $crud_master = $crm . "Rudra_map_crtl/";
        $route['rudra_map'] = $crud_master . 'index';
        $route['rudra_map/index'] = $crud_master . 'index';
        $route['rudra_map/list'] = $crud_master . 'list';
        $route['rudra_map/post_actions/(:any)'] = $crud_master.'post_actions/$1';
	**************/
    ...
    public function index()
    {
        ...
        $data['pageTitle'] = ' Map';                        
        $data['page_template'] = '_rudra_map';
        $data['page_header'] = ' Map';
        $data['load_type'] = 'all';                
    ...
    public function list()
    {
        ...
        $orderArray = array(  'id', 'image', 'status', 'is_deleted', );
		...
		if(!empty($rows))
		{
			...
			foreach ($rows as $row)
			{
				$actions_base_url = 'rudra_map/post_actions';
				$actions_query_string = "?id= $row->id.'&target_table='.$row->id";
				$form_data_url = 'rudra_map/post_actions';
				$action_url = 'rudra_map/post_actions';
                ...
                $row->image = '<a href="'.base_url().$row->image.'" target="_blank"><img src="'.base_url().$row->image.'" class="rounded mx-auto d-block float-left" alt="Map view image" style="width: 50px; height: 50px;"></a>';
    ...
    public function post_actions($param1)
    {
        ...
        if($action == "get_data")
        {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_map_form", $data, TRUE);
        ...
        if($action == "update_data")
        {
            if(!empty($_POST))
            {
                ...
                if (isset($_FILES['image']) && $_FILES['image']['name'] != '') {
                    $bannerpath = 'app_assets/images/admin';
                    $thumbpath = 'app_assets/images/admin';
                    ...
                    $up_array = array(
                        'image' => $bannerpath . '/' . $uploadedImage['file_name']
                    );
                    $this->db->where('id', $id);
        ...
        if($action == "export_data")
        {
            ...
            $filename = strtolower('rudra_map').'_'.date('d-m-Y').".csv";
