...
class Rudra_wash_comments_crtl extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('rudra_wash_comments_rudra_model', 'rudram');
    ...
    /***********
	//Rudra_wash_comments_crtl ROUTES
        $crud_master = $crm . "Rudra_wash_comments_crtl/";
        $route['rudra_wash_comments'] = $crud_master . 'index';
        $route['rudra_wash_comments/index'] = $crud_master . 'index';
        $route['rudra_wash_comments/list'] = $crud_master . 'list';
        $route['rudra_wash_comments/post_actions/(:any)'] = $crud_master.'post_actions/$1';
     **************/

    public function index()
    {
        // main index codes goes here
        $data['pageTitle'] = ' Wash Comments';
        $data['page_template'] = '_rudra_wash_comments';
        $data['page_header'] = ' Wash Comments';
        $data['load_type'] = 'all';
    ...
    public function list()
    {
        ...
        $orderArray = array('id', 'fk_user_id', 'fk_wash_id', 'comment', 'status', 'is_deleted',);
        ...
        if (!empty($rows)) {
            ...
            foreach ($rows as $row) {
                $actions_base_url = 'rudra_wash_comments/post_actions';
                $actions_query_string = "?id= $row->id.'&target_table='.$row->id";
                $form_data_url = 'rudra_wash_comments/post_actions';
                $action_url = 'rudra_wash_comments/post_actions';
                ...
                // $b_wash_id = (isset($row->b_wash_id) && $row->b_wash_id != "") ? washTypesList($id = $row->b_wash_id) : [];
                // $row->fk_wash_id = (!empty($b_wash_id) && isset($b_wash_id['name'])) ? $b_wash_id['name'] : "NA";
                $row->fk_wash_id = (isset($row->booking_id) && $row->booking_id != "") ? '#' . $row->booking_id : "NA";
    ...
    public function post_actions($param1)
    {
        ...
        if ($action == "get_data") {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_wash_comments_form", $data, TRUE);
        ...
        if ($action == "export_data") {
            ...
            $filename = strtolower('rudra_wash_comments') . '_' . date('d-m-Y') . ".csv";
