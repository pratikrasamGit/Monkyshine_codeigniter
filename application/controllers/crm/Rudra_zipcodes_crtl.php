...
class Rudra_zipcodes_crtl extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->bdp = $this->db->dbprefix;
        $this->full_table = 'rudra_zipcodes';
        $this->return_data = array('status' => false, 'msg' => 'Error', 'login' => false, 'data' => array());
        //$this->set_data();
        $this->base_id = 0;
        $this->load->model('crudmaster_model', 'crd');
        $this->load->model('rudra_zipcodes_rudra_model', 'rudram');
	...
	//Rudra_zipcodes_crtl ROUTES
        $crud_master = $crm . "Rudra_zipcodes_crtl/";
        $route['rudra_zipcodes'] = $crud_master . 'index';
        $route['rudra_zipcodes/index'] = $crud_master . 'index';
        $route['rudra_zipcodes/list'] = $crud_master . 'list';
        $route['rudra_zipcodes/post_actions/(:any)'] = $crud_master.'post_actions/$1';
     **************/

    public function index()
    {
        // main index codes goes here
        $data['pageTitle'] = ' Zipcodes';
        $data['page_template'] = '_rudra_zipcodes';
        $data['page_header'] = ' Zipcodes';
        $data['load_type'] = 'all';
    ...
    public function list()
    {
        ...
        $orderArray = array('id', 'zipcode', 'fee','status', 'is_deleted',);
        ...
        if (!empty($rows)) {
            $res_json = array();
            foreach ($rows as $row) {
                $actions_base_url = 'rudra_zipcodes/post_actions';
                $actions_query_string = "?id= $row->id.'&target_table='.$row->id";
                $form_data_url = 'rudra_zipcodes/post_actions';
                $action_url = 'rudra_zipcodes/post_actions';
                ...
                $row->service = "Service Available";
                if($row->is_deleted == '1' || $row->status == '0' ){
                    $row->service = "Service Not Available";
                }
    ...
    public function post_actions($param1)
    {
        ...
        if ($action == "get_data") {
            ...
            $html = $this->load->view("crm/forms/_ajax_rudra_zipcodes_form", $data, TRUE);
        ...
        if ($action == "export_data") {
            ...
            $filename = strtolower('rudra_zipcodes') . '_' . date('d-m-Y') . ".csv";
