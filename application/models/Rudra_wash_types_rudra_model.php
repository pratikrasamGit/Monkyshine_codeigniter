...
class Rudra_wash_types_rudra_model extends CI_Model
{
	...
	public function get_table_data($limit, $start, $order, $dir, $filter_data, $tbl_data, $table_name)
	{
		...
		$query = " SELECT TBL.id, TBL.car_size, TBL.icon, TBL.wash_name, TBL.amount, TBL.description, TBL.status, TBL.is_deleted";
		$query .= "  FROM $table ";
		$where = " WHERE 1 = 1 ";
		$where .= " AND ( ";
		$where .=  "  TBL.id LIKE '%" . $filter_data['general_search'] . "%'";
		$where .=  "  OR  TBL.car_size LIKE '%" . $filter_data['general_search'] . "%'";
		$where .=  "  OR  TBL.icon LIKE '%" . $filter_data['general_search'] . "%'";
		$where .=  "  OR  TBL.wash_name LIKE '%" . $filter_data['general_search'] . "%'";
		$where .=  "  OR  TBL.amount LIKE '%" . $filter_data['general_search'] . "%'";
