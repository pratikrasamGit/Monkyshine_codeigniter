...
class Rudra_zipcodes_rudra_model extends CI_Model
                
{
	...
	$query = " SELECT TBL.id, TBL.zipcode,TBL.fee, TBL.status, TBL.is_deleted" ;
	$query .= "  FROM $table "; 
	$where = " WHERE 1 = 1 ";
	 $where .= " AND ( ";
	$where .=  "  TBL.id LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.zipcode LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.fee LIKE '%".$filter_data['general_search']."%'";
