...
class Rudra_threshold_rudra_model extends CI_Model
                
{
	...
	$query = " SELECT TBL.id, TBL.label, TBL.value, TBL.status, TBL.is_deleted" ;
	$query .= "  FROM $table "; 
	$where = " WHERE 1 = 1 ";
	 $where .= " AND ( ";
	$where .=  "  TBL.id LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.label LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.value LIKE '%".$filter_data['general_search']."%'";
