...
class Rudra_chat_rudra_model extends CI_Model
                
{
	...
	$query = " SELECT TBL.id, TBL.fk_from_id, TBL.fk_to_id, TBL.message, TBL.status, TBL.is_deleted" ;
	$query .= "  FROM $table "; 
	$where = " WHERE 1 = 1 ";
	 $where .= " AND ( ";
	$where .=  "  TBL.id LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.fk_from_id LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.fk_to_id LIKE '%".$filter_data['general_search']."%'";
