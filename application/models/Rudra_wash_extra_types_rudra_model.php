...
class Rudra_wash_extra_types_rudra_model extends CI_Model
                
{
	...
	$query = " SELECT TBL.id, TBL.fk_wash_id, TBL.extra_name, TBL.amount, TBL.status, TBL.is_deleted" ;
	$query .= "  FROM $table "; 
	$where = " WHERE 1 = 1 ";
	 $where .= " AND ( ";
	$where .=  "  TBL.id LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.fk_wash_id LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.extra_name LIKE '%".$filter_data['general_search']."%'";
