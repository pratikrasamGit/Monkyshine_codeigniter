...
class Rudra_chat_uploads_rudra_model extends CI_Model
                
{
	...
	$query = " SELECT TBL.id, TBL.fk_chat_id, TBL.file_name, TBL.status, TBL.is_deleted" ;
	$query .= "  FROM $table "; 
	$where = " WHERE 1 = 1 ";
	 $where .= " AND ( ";
	$where .=  "  TBL.id LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.fk_chat_id LIKE '%".$filter_data['general_search']."%'";
