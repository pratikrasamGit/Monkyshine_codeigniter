...
class Rudra_user_rudra_model extends CI_Model
{
	...
	public function get_table_data($limit, $start, $order, $dir, $filter_data, $tbl_data, $table_name)
	{
		...
		$query = " SELECT TBL.id, TBL.user_type, TBL.login_type, TBL.email, TBL.password, TBL.name, TBL.surname, TBL.contact_number, TBL.lat, TBL.lon, TBL.address, TBL.zipcode, TBL.city, TBL.status, TBL.is_deleted";
		$query .= "  FROM $table ";
		$where = " WHERE 1 = 1 ";
		$where .= " AND ( ";
	...
	public function checkSocialUserExists($social_id = "")
	{
		$return = [];
		if ($social_id != "") {
	...
	public function getUserRegisteredCarNumbers($user_id = "", $id = "")
	{
		...
		if ($user_id != "") {
			...
			$sql = "SELECT * FROM `" . $this->registration_number_table . "` " . $where . " AND `is_deleted` = '0' AND `status` = '1' ORDER BY `id` DESC;";
			$check_user_exists = $this->db->query($sql);
			if ($check_user_exists->num_rows() > 0) {
	...
	public function chatData($user_id, $page = "1", $chat_id = "")
	{
		$limit = "25";
		$start = "0";
		if ($page > 1) $start = ($page - 1) * $limit;
		...
		$query = "SELECT * FROM `rudra_chat` WHERE (fk_from_id = '" . $user_id . "' OR `fk_to_id` = '" . $user_id . "') AND `status` = '1' AND `is_deleted` = '0' " . $chat . " ORDER BY `created_at` DESC LIMIT $start , $limit";
		$result = $this->db->query($query);

		/* getting page count */
		$query1 = "SELECT * FROM `rudra_chat` WHERE (fk_from_id = '" . $user_id . "' OR `fk_to_id` = '" . $user_id . "') AND `status` = '1' AND `is_deleted` = '0' " . $chat . " ORDER BY `created_at` DESC;";
		...
		$user_info = $this->db->get($this->user_table);
		$user_name = "User";
		...
		if ($result->num_rows() > 0) {
			foreach ($result->result() as $res) {
				...
				/* to data */
				$chat_array["message"] = $res->message;
	...
	public function updateNotifications($user_id, $message)
	{
		$update_array = array('fk_user_id' => $user_id, 'messge' => $message);
