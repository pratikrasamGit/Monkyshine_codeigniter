...
class Rudra_notifications_rudra_model extends CI_Model
                
{
	...
	$query = " SELECT TBL.id, TBL.fk_user_id, TBL.messge, TBL.status, TBL.is_deleted" ;
	$query .= "  FROM $table "; 
	$where = " WHERE 1 = 1 ";
	 $where .= " AND ( ";
	$where .=  "  TBL.id LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.fk_user_id LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.messge LIKE '%".$filter_data['general_search']."%'";
