...
class Rudra_washer_bank_info_rudra_model extends CI_Model
{
	...
	public function get_table_data($limit, $start, $order, $dir, $filter_data, $tbl_data, $table_name)
	{
		...
		$query = " SELECT TBL.id, TBL.fk_user_id, TBL.registration_number, TBL.account_number, TBL.status, TBL.is_deleted, u.id u_id, u.name u_name";
		$query .= "  FROM $table ";
		$query .= "  LEFT JOIN `rudra_user` u ON u.id = TBL.fk_user_id ";
		$where = " WHERE 1 = 1 ";
		$where .= " AND ( ";
