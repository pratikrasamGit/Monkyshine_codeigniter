...
class Rudra_faq_rudra_model extends CI_Model
                
{
	...
	$query = " SELECT TBL.id, TBL.question, TBL.answer,TBL.question_dk, TBL.answer_dk, TBL.status, TBL.is_deleted" ;
	$query .= "  FROM $table "; 
	$where = " WHERE 1 = 1 ";
	 $where .= " AND ( ";
	$where .=  "  TBL.id LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.question LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.answer LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.question_dk LIKE '%".$filter_data['general_search']."%'";
	$where .=  "  OR  TBL.answer_dk LIKE '%".$filter_data['general_search']."%'";
