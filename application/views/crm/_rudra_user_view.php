                    ...
                    <tbody>
                        <?php foreach($booking_data as $value){ ?>
                            <tr>
                                ...
                                <td>
                                    <?php if($value->wash_status == '3'){ 
                                            if($value->pay_method == '1'){ echo "Credit card"; }else{ echo "Mobilepay"; }  
                                            }else{ echo "--"; } ?>
                                </td>
                                <td>
                                    <?php   if($value->washer_id != ''){ 
                                                $washer = $this->db->get_where('rudra_user', array('id' => $value->washer_id))->row();
                                                echo $washer->name.' '.$washer->surname;
                                            }else{ echo "--"; } ?>
                                </td>
                            </tr>
                        <?php } ?>
                ...
                <table class="table">
                    <tbody>
                        <?php foreach($car_data as $car){ ?>
                            <tr>
                                <td><a target="_blank" href="<?=base_url()?>rudra_user_registered_car_numbers/view/<?=$car->id?>"><?=$car->registration_number?></a></td>
                                <td><?=$car->name?></td>
                            </tr>
                        <?php } ?>
                    </tbody>
