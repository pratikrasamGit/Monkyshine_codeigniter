...
<div class="form-group col-sm-12">
	<label>Day</label>
 	<input type="date"  name="day" class="form-control" value="<?= (!empty($form_data) ? $form_data->day : ''); ?>"   placeholder="Day"   />
</div>

<?php for($i=1;$i<=6;$i++){ ?>
<div class="form-group col-sm-4">
	<label>Time</label>
 	<input type="time"  name="time<?=$i?>" class="form-control" value="<?= (!empty($form_data) ? $form_data->time : $times[$i-1]); ?>"   placeholder="Time"   />
</div>

<div class="form-group col-sm-4">
	<label>Slots Taken</label>
 	<input type="number"  name="slots_taken<?=$i?>" class="form-control" value="<?= (!empty($form_data) ? $form_data->slots_taken : '0'); ?>"   placeholder="Slots Taken"   />
</div>
<div class="form-group col-sm-4">
	<label>Total Slots</label>
 	<input type="number"  name="total_slots<?=$i?>" class="form-control" value="<?= (!empty($form_data) ? $form_data->total_slots : '1'); ?>"   placeholder="Total Slots"   />
</div>
<?php } ?>
