            ...
            <div class="card-header">
                <div class="row">
                    ...
                    <div class="col-sm-7 text-right">
                        <button onclick="static_form_modal('rudra_notifications/post_actions/get_data?id=0','rudra_notifications/post_actions/insert_data','md','Update Details')" type="button" class="btn btn-glow-success btn-info" title="" data-toggle="tooltip" data-original-title="Add New Item" aria-describedby="tooltip651610">New</button>&nbsp;
...
<!--- FORM Modal Ends ---->
<div class="modal fade dynamic-modal" id="form_modal_rudra_notifications">
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header theme-bg2 ">
                <h5 class="modal-title text-white" id="heading_rudra_notifications">Loading...</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
            </div>
            <form id="frm_rudra_notifications" method="post" enctype="multipart/form-data">
                <div class="modal-body" id="modal_form_data_rudra_notifications"> </div>
...
<script type="text/javascript">
    ...
    function fill_server_data_table(search_json) {
        usrTable = $('.user_post').DataTable({
            ...
            responsive: true,
            "ajax": {
                "url": "<?php echo base_url('rudra_notifications/list') ?>",
                "dataType": "json",
                "type": "POST",
                "data": {
...
<script type='text/javascript'>
    function static_form_modal(data_url, action_url, mtype, heading) {
        $("#form_modal_rudra_notifications").modal();
        $("#form_modal_rudra_notifications").addClass('md-show');
        $("#heading_rudra_notifications").text(heading);
        $("#frm_rudra_notifications").attr("action", '<?= base_url() ?>' + action_url);
        $.ajax({
            type: 'POST',
            ...
            success: function(data) {
                ...
                if (response.status) {
                    ...
                    $("#modal_form_data_rudra_notifications").html(jsonData.form_data);
    ...
    $("#frm_rudra_notifications").submit(function(e) {
        e.preventDefault();
        $(".btn-modal-form").prop('disabled', true);
        $(".btn-modal-form").html('Wait...');
        var formData = new FormData($("#frm_rudra_notifications")[0]);
        var action_url = $("#frm_rudra_notifications").attr("action");
        $.ajax({
            type: 'POST',
            url: action_url,
            data: formData,
